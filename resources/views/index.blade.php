...
<html lang="en">
    ...
    <body class="bg-gray-100 tracking-wide bg-gray-200">
        ...
        <div id="app" v-cloak class="w-full flex lg:pt-10">
            <aside class="text-sm ml-1.5 text-grey-darkest break-all bg-gray-200 pl-2 pb-12 h-screen sticky top-1 overflow-auto w-10/12">
                ...
                <section class="pt-5 pl-2 pr-2 pb-5 border mb-10 rounded bg-white shadow">
                    <div class="font-sans">
                        ...
                        <prism-editor class="my-prism-editor" style="max-width:100%;min-height:100px;background:#2d2d2d;color: #ccc;resize:both" v-model="requestHeaders" :highlight="highlighter" line-numbers></prism-editor>
                    </div>
                </section>
                ...
                <table class="table-fixed text-sm mt-5 mb-5" style="width: max-content">
                    <tbody>
                        ...
                        <tr>
                            <td>
                                <h2 class="font-bold mt-6 text-lg">{{$tag}}</h2>
                            </td>
                        </tr>
                        ...
                        <tr v-if="!docs[{{$index}}]['isHidden']">
                            <td style="padding: 8px 0;border-bottom:1px solid #333">
                                <a href="#{{$doc['methods'][0] .'-'. $doc['uri']}}" @click="highlightSidebar({{$index}})">
                                    <span class="font-medium inline-flex items-center justify-center px-2 py-1 text-xs font-bold leading-none rounded text-{{in_array('GET', $doc['methods']) ? 'green': ''}}-100 bg-{{in_array('GET', $doc['methods']) ? 'green': ''}}-500 text-{{in_array('POST', $doc['methods']) ? 'black': ''}} bg-{{in_array('POST', $doc['methods']) ? 'red': ''}}-500 text-{{in_array('PUT', $doc['methods']) ? 'black': ''}}-100 bg-{{in_array('PUT', $doc['methods']) ? 'yellow': ''}}-500 text-{{in_array('PATCH', $doc['methods']) ? 'black': ''}}-100 bg-{{in_array('PATCH', $doc['methods']) ? 'yellow': ''}}-500 text-{{in_array('DELETE', $doc['methods']) ? 'white': ''}} bg-{{in_array('DELETE', $doc['methods']) ? 'black': ''}}">
                                        {{$doc['methods'][0]}}
                                    </span>
                                    <span class="text-md" v-bind:class="docs[{{$index}}]['isActiveSidebar'] ? 'font-bold':''">
                                        ...
                                        <span class="font-bold text-green-600 border rounded-full pr-1 pl-1 border-green-600" v-if="docs[{{$index}}]['responseOk'] === true">
                                            {{$doc['uri']}} -
                                            <span class="inline-flex text-xs" v-text="'Status:'+docs[{{$index}}]['responseCode'] + ', Took:' + docs[{{$index}}]['responseTime'] + 'ms'">
                                            </span>
                                        </span>
            ...
            <div class="ml-6 mr-6 pl-2 w-2/3 p-2" style="width: 100%">
                ...
                <section class="pt-5 pl-2 pr-2 pb-5 border-2 mb-10 rounded bg-white shadow" v-if="!docs[{{$index}}]['isHidden']">
                    ...
                    <table class="table-fixed text-sm mt-5">
                        <tbody>
                            <tr>
                                <td class="align-left pl-2 pr-2 bg-gray-100 border-r-2">Tag</td>
                                <td class="align-left pl-2 pr-2 font-bold break-all">{{$doc['tags']}}</td>
                            </tr>
                            <tr>
                    ...
                    @endif
                    <button class="hover:bg-red-500 font-semibold hover:text-white mt-2 pl-5 pr-5 border-gray-700 hover:border-transparent shadow-inner border-2 rounded-full" v-if="!docs[{{$index}}]['try']" v-on:click="docs[{{$index}}]['try'] = !docs[{{$index}}]['try'];docs[{{$index}}]['cancel'] = !docs[{{$index}}]['cancel']">
                        Try
                    </button>

                    <button class="hover:bg-red-500 font-semibold hover:text-white mt-2 pl-5 pr-5 border-gray-700 hover:border-transparent shadow-inner border-2 rounded-full" v-if="!docs[{{$index}}]['cancel']" v-on:click="docs[{{$index}}]['cancel'] = !docs[{{$index}}]['cancel'];docs[{{$index}}]['try'] = !docs[{{$index}}]['try'];">
                        Cancel
                    </button>

                    <button v-if="docs[{{$index}}]['try']" @click="request(docs[{{$index}}])" class="bg-blue-500 hover:bg-blue-700 text-white font-bold mt-2 border-blue-800 border-2 shadow-inner mb-1 pl-5 pr-5 rounded-full">
                    ...
                    <div class="grid grid-cols-1 mt-3 pr-2 overflow-auto">
                        <div class="">
                            <div v-if="docs[{{$index}}]['try']">
                                ...
                                <prism-editor class="my-prism-editor" style="padding:15px 0px 15px 0px;min-height:20px;background:#2d2d2d;color: #ccc;resize:both;" v-model="docs[{{$index}}]['url']" :highlight="highlighter" line-numbers></prism-editor>
                                <br>
                        ...
                        <div class="grid grid-cols-2 gap-2" v-if="docs[{{$index}}]['response'] && !docs[{{$index}}]['cancel']">
                            <div class="border-r-2">
                                ...
                                <table class="table-fixed text-sm mt-5">
                                    <tbody>
                                        <tr>
                                            ...
                                            <td class="align-left pl-2 pr-2 break-all">
                                                <span v-if="docs[{{$index}}]['responseOk']" class="inline-flex  text-xs font-bold leading-none text-green-700">
                                                    SUCCESS
                                                </span>
                                                <span v-if="!docs[{{$index}}]['responseOk']" class="inline-flex items-center justify-center px-2 py-1 text-xs font-bold leading-none text-red-100 bg-red-500 rounded">
                                                    ERROR
                                                </span>
                                            </td>
                                        </tr>
                                        <tr>
                                            ...
                                            <td class="align-left pl-2 pr-2 break-all">
                                                <span v-if="docs[{{$index}}]['responseOk']" class="inline-flex text-xs font-bold text-green-900" v-text="docs[{{$index}}]['responseCode']">
                                                </span>
                                                <span v-if="!docs[{{$index}}]['responseOk']" class="inline-flex text-xs font-bold text-red-900" v-text="docs[{{$index}}]['responseCode']">
                                                </span>
                                            </td>
                                        </tr>
                                        <tr>
                                            ...
                                            <td class="align-left pl-2 pr-2 break-all">
                                                <span v-if="docs[{{$index}}]['memory']" class="inline-flex text-xs font-bold text-red-900" v-text="docs[{{$index}}]['memory']">
                                                </span>
                                            </td>
                                        </tr>
                                        <tr>
                                            ...
                                            <td class="align-left pl-2 pr-2 break-all">
                                                <span v-if="docs[{{$index}}]['responseTime']" class="inline-flex items-center justify-center px-2 py-1 text-xs font-bold leading-none text-gray-800 bg-yellow-400 rounded" v-text="docs[{{$index}}]['responseTime'] + 'ms'">
                                                </span>
                                            </td>
                                        </tr>
                                    </tbody>
                                ...
                                <prism-editor v-if="docs[{{$index}}]['responseHeaders']" class="my-prism-editor shadow-inner border-gray-400 border-2 rounded" style="width:75%;min-height:100px;height:200px;background:#fefefe;color: rgb(66, 63, 63);resize:both" readonly v-model="docs[{{$index}}]['responseHeaders']" :highlight="highlighterAtom" line-numbers></prism-editor>
                            </div>
                            <div>
                                <p class="text-xs pb-2 font-medium text-gray-800">Response from the server</p>
                                <prism-editor v-if="docs[{{$index}}]['response']" class="my-prism-editor shadow-inner border-gray-400 border-2 rounded" style="min-height:100px;height:300px;background:#2d2d2d;color: #ccc;resize:both" readonly v-model="docs[{{$index}}]['response']" :highlight="highlighter" line-numbers></prism-editor>
                            </div>
                            <div class="border-r-2">
                            ...
                            <div class="">
                                ...
                                <div v-for="(query, index) in docs[{{$index}}]['queries']">
                                    ...
                                    <prism-editor v-model="sqlFormatter.format(query['sql'])" class="my-prism-editor" style="padding:10px 0px 10px 0px;min-height:20px;height:350px;background:rgb(52 33 33);color: #ccc;resize:both;" readonly :highlight="highlighter" line-numbers></prism-editor>
                                </div>
                            </div>
                        </div>
                    </div>
